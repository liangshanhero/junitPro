...
public class NewTestCaseWizardPageOne extends NewTypeWizardPage {
	...
	//private MethodStubsSelectionButtonGroup fAssertMethodButtons;
//	private Text testCaseArea;
//	private Button bAssertEquals;
//	private Button bAssertFalse;
//	private Button bAssertTrue;
//	private Button bAssertNull;
	...
	public void createControl(Composite parent) {
		...
		createSeparator(composite, nColumns);
		//createTextArea(composite, nColumns);
...
/*
	public void createTextArea(Composite composite, int nColumns) {
//		Text t = new Text(composite, SWT.MULTI | SWT.BORDER | SWT.WRAP | SWT.V_SCROLL);
//		t.setLayoutData(new GridData(GridData.FILL_BOTH));
		testCaseArea = new Text(composite, SWT.MULTI | SWT.BORDER | SWT.WRAP | SWT.V_SCROLL);
		testCaseArea.setEnabled(true);
		testCaseArea.setFont(composite.getFont());
		//testCaseArea.setText("fuck");
...
//		String[] buttonNames= new String[] {"bAssertEquals()","bAssertFalse()","bAssertTrue()","bAssertNull()"};
//		fAssertMethodButtons= new MethodStubsSelectionButtonGroup(SWT.CHECK, buttonNames, 2);
//		//fAssertMethodButtons.setLabelText(WizardMessages.NewTestCaseWizardPageOne_method_Stub_label);
//		fAssertMethodButtons.setEnabled(true);
//		LayoutUtil.setHorizontalSpan(fAssertMethodButtons.getSelectionButtonsGroup(composite), nColumns - 1);
		
		bAssertEquals = new Button(composite, SWT.CHECK);
		bAssertEquals.setText("Assert&Equals()");
		bAssertEquals.setEnabled(true);
		
		bAssertFalse = new Button(composite, SWT.CHECK);
		bAssertFalse.setText("Assert&False()");
		bAssertFalse.setEnabled(true);
		
//		String msg = "";
//		msg += composite.getToolTipText() + "||" + composite.getBackground() + "||" + composite.getSize();
//		JOptionPane.showMessageDialog(null, msg);
	...
	*/
	...
	protected void createTypeMembers(IType type, ImportsManager imports, IProgressMonitor monitor) throws CoreException {
		createTestObjects(type, imports);
	...
	private void createTestObjects(IType type, ImportsManager imports) throws JavaModelException{
		IType classUnderTest = fPage2.getClassUnderTest();
		type.createField("private " + classUnderTest.getFullyQualifiedName() + " " + classUnderTest.getElementName().toLowerCase() + ";", null, true, null);
	}
	...
	private void createSetupStubs(IType type, String methodName, boolean isStatic, String annotationType, ImportsManager imports) throws CoreException {
		...
		if (methodTemplate != null) {
			...
			buffer.append(" {"); //$NON-NLS-1$
			if(methodName.equals("setUpBeforeClass") || methodName.equals("setUp"))
				buffer.append(Messages.format("{0} = new {1}();", new Object[]{fPage2.getClassUnderTest().getElementName().toLowerCase(), fPage2.getClassUnderTest().getFullyQualifiedName()}));
			buffer.append("}");
			buffer.append(delimiter);
			content= buffer.toString();
		}
	...
	private void createTestMethodStubs(IType type, ImportsManager imports) throws CoreException {
		...
		for (int i = 0; i < methods.length; i++) {
			...
			buffer.append("()");//$NON-NLS-1$
			appendTestMethodBody(buffer, type.getCompilationUnit(), testName);
	...
	private void appendTestMethodBody(StringBuffer buffer, ICompilationUnit targetCu, String testName) throws CoreException {
		final String delimiter= getLineDelimiter();
		...
		IMethod[] methods= fPage2.getCheckedMethods();
		IType classUnderTest = fPage2.getClassUnderTest();
		String className = classUnderTest.getElementName().toLowerCase();
		boolean flag = true;
		for (int i = 0; i < methods.length; i++) {
//			IMethod method= methods[i];
			String methodName= methods[i].getElementName();
			if(!testName.equalsIgnoreCase("test" + methodName))
				continue;
//			StringBuffer name= new StringBuffer(PREFIX).append(Character.toUpperCase(elementName.charAt(0))).append(elementName.substring(1));
			//msg += methods[i].getElementName() + "\n";
			if(fPage2.getAssertEqualsButton().getSelection()){
				String text = fPage2.getAssertEqualsArea().getText();
				StringTokenizer token = new StringTokenizer(text);
				String methodInTextArea = "";
				//String firstTokenString = "";
				while(token.hasMoreTokens()){
					methodInTextArea = token.nextToken();
					if(methodInTextArea.equalsIgnoreCase(methodName)){
						while(token.hasMoreTokens()){
							String methodArgs = token.nextToken();
							if(methodArgs.equals("#"))
								break;
							else if(methodArgs.equals("$"))
								buffer.append(Messages.format("assertEquals({0}.{1}(),{2});", new Object[]{className, methodName, token.nextToken()}));
							else if(token.hasMoreTokens())
								buffer.append(Messages.format("assertEquals({0}.{1}({2}),{3});", new Object[]{className, methodName, methodArgs, token.nextToken()}));
						}
					}
			...
			if(fPage2.getAssertFalseButton().getSelection()){
				String text = fPage2.getAssertFalseArea().getText();
				StringTokenizer token = new StringTokenizer(text);
				String methodInTextArea = "";
				while(token.hasMoreTokens()){
					methodInTextArea = token.nextToken();
					if(methodInTextArea.equalsIgnoreCase(methodName)){
						while(token.hasMoreTokens()){
							...
							if(methodArgs.equals("#"))
								...
								buffer.append(Messages.format("assertFalse({0}.{1}());", new Object[]{className, methodName}));
							else if(token.hasMoreTokens())
								buffer.append(Messages.format("assertFalse({0}.{1}({2}));", new Object[]{className, methodName, methodArgs}));
						}
					}
				}
			}
			if(fPage2.getAssertNullButton().getSelection()){
				String text = fPage2.getAssertNullArea().getText();
				StringTokenizer token = new StringTokenizer(text);
				String methodInTextArea = "";
				while(token.hasMoreTokens()){
					methodInTextArea = token.nextToken();
					if(methodInTextArea.equalsIgnoreCase(methodName)){
						while(token.hasMoreTokens()){
							...
							if(methodArgs.equals("#"))
								...
								buffer.append(Messages.format("assertNull({0}.{1}());", new Object[]{className, methodName}));
							else if(token.hasMoreTokens())
								buffer.append(Messages.format("assertNull({0}.{1}({2}));", new Object[]{className, methodName, methodArgs}));
						}
					}
				}
			}
			if(fPage2.getAssertSameButton().getSelection()){
				String text = fPage2.getAssertSameArea().getText();
				StringTokenizer token = new StringTokenizer(text);
				String methodInTextArea = "";
				while(token.hasMoreTokens()){
					methodInTextArea = token.nextToken();
					if(methodInTextArea.equalsIgnoreCase(methodName)){
						while(token.hasMoreTokens()){
							...
							if(methodArgs.equals("#"))
								...
								buffer.append(Messages.format("assertSame({0}.{1}(),{2});", new Object[]{className, methodName, token.nextToken()}));
							else if(token.hasMoreTokens())
								buffer.append(Messages.format("assertSame({0}.{1}({2}),{3});", new Object[]{className, methodName, methodArgs, token.nextToken()}));
						}
					}
				}
			}
//				if(fPage2.getAssertFalseButton().getSelection())
//				buffer.append(Messages.format("assertFalse({0});", fPage2.getAssertFalseArea().getText()));
//			if(fPage2.getAssertTrueButton().getSelection())
//				buffer.append(Messages.format("assertTrue({0});",  fPage2.getAssertTrueArea().getText()));
//			if(fPage2.getAssertNullButton().getSelection())
//				buffer.append(Messages.format("assertNull({0},{1});",  new Object[]{methods[i].getElementName(), fPage2.getAssertNullArea().getText()}));
		}
		
		//JOptionPane.showMessageDialog(null, msg);
		//JOptionPane.showMessageDialog(null, testCaseArea.getText());
		
//		String msg = "1,1";
//		buffer.append(Messages.format("assertEquals({0});", msg));
